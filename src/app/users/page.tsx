        ...
        <div className="max-w-5xl mx-auto p-6">
            ...
            <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {users.map((user: IUser) => (
                    <li key={user.id} className="bg-white shadow-md p-4 rounded-lg hover:shadow-lg transition">
                        <Link href={`/users/${user.id}`} className="flex flex-col items-center">
                            {user.image && <img src={user.image} alt={user.lastName} className="w-20 h-20 rounded-full mb-2" />}
                            <span className="text-lg font-semibold">{user.id}. {user.firstName} {user.lastName}</span>
                        </Link>
                    </li>
                ))}
            </ul>
